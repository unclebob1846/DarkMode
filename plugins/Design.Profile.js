...
window[window.dID][window.dID+"a"]("changePostLike", function(postId, isComment) 
{
	if (this.config.Design.Profile.posts[postId] != null)
	{
		if (this.config.Design.Profile.posts[postId].liked)
		{
			...
			this.config.Design.Profile.posts[postId].element.html('<img src="'+this.config.Design.images.likes+'" />'+this.language.like+''+(this.config.Design.Profile.posts[postId].likes > 0 ? this.language.otherLikes.replace('%1', this.config.Design.Profile.posts[postId].likes): ""));
		...
		else 
		{
			...
			this.config.Design.Profile.posts[postId].likes++;
			this.config.Design.Profile.posts[postId].element.find(".like").html('<img src="'+this.config.Design.images.thumbsDown+'" />'+this.language.unlike+''+(this.postLikesCount[postId] > 1 ? this.language.otherLikes.replace('%1', this.config.Design.Profile.posts[postId].likes - 1): ""));
...
window[window.dID][window.dID+"a"]("openProfile", function(username, onlyViewer) 
{
